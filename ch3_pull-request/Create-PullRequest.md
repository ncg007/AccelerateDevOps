...
4. Added a new line of text here.

5. Add one or two lines here with a random text:

6. RCMS upgrade is a key project we started at NCG for our client HPE
7. We are ensuring that we will be winning this project at any cost

   __...__

8. Modify the following line by removing the letters that do not belong:

   __---> The cow jumped over the moon__

9. Commit your changes into a new _branch_:

   ![Commit Changes](img/commit.png)

10. In the next screen create the pull request by completing the title and description. Note the complete markdown support that we've seen in Chapter 2 for issues:
...
11. Got to `Files changed` and note the changes (deleted lines red, added lines green). Add a comment to a line by clicking the `+`at the beginning of the line when hovering with the mouse over it. Add a comment to line nine and click `Add single comment`.
